...
cs_ads = pd.DataFrame(engine.connect().execute(text(sql)))
# cs_ads = pd.DataFrame(sql, engine)
...
if len(rows_selected)>0:
    ...
    full_styles = pd.DataFrame(engine.connect().execute(text(style_sql)))
    ...
    if change_style:
         ...
         if all_styles == True or pos_id ==6:
             new_style_ = 'null'
             new_style_id = 'null'
         else:
